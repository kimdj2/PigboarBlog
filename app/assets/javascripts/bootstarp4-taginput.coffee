  ...
  TagsInput.prototype =
    ...
    add: (item, dontPushVal, options) ->
      ...
      if existing and !self.options.allowDuplicates
        ...
        if self.options.onTagExists
          $existingTag = $('.badge', self.$container).filter(->
      ...
      # add a tag element
      $tag = $('<span class="badge ' + htmlEncode(tagClass) + (if itemTitle != null then '" title="' + itemTitle else '') + '">' + htmlEncode(itemText) + '<span data-role="remove"></span></span>')
      $tag.data 'item', item
      self.findInputWrapper().before $tag
      $tag.after ' '
      # Check to see if the tag exists in its raw or uri-encoded form
      optionExists = $('option[value="' + encodeURIComponent(itemValue) + '"]', self.$element).length or $('option[value="' + htmlEncode(itemValue) + '"]', self.$element).length
      # add <option /> if item represents a value not present in one of the <select />'s options
      if self.isSelect and !optionExists
    ...
    build: (options) ->
      ...
      self.$container.on 'keydown', 'input', $.proxy(((event) ->
        ...
        $input.attr 'size', Math.max(@inputSize, size)
        return
      ), self)
